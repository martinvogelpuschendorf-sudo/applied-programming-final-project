...
3. Press the connect button to start live streaming.
4. Use `Disconnect` to stop the client connection. The received data remains
   available in the `Offline` tab for inspection.

The GUI also includes a built-in local TCP server. The `Start TCP Server` button
launches this local demo server on the selected host and port. This is useful
for development, debugging, or presenting the app without separately launching
the provided exercise server, but it is not the required final-project server.
If a different free port is entered, the local demo server can be started on
that port and the GUI client can connect to it, which is why ports other than
the default may still appear to work.
